...
K runtest(K opv, K ecxv, K eaxv, K edxv, K testCount, K cvLibName)
{
	...
	if (cvLibName->t != KC) {
		krr("type.libname.ne.10");
		return (K)0;
	}
	for (i = 0 ; i < cvLibName->n ; i++) {
		libname[i] = kG(cvLibName)[i];
	}
	libname[i] = (char)0;
	...
	if ((error = dlerror()) != NULL) {
		krr("While locating symbol 'execute_test'");
		return (K)0;
	}
